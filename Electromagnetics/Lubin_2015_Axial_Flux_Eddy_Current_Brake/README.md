...
The air gap thickness is chosen as $`g = 3\,\mathrm{mm}`$ (the reference considers $`g = 3\,\mathrm{mm}`$, $`5\,\mathrm{mm}`$, 
...
We run the simulations for 20 time steps with a time step size of $`\Delta t = 5 \times 10^{-4}\,\mathrm{s}`$,
after which we increase the rotation rate of the plate and continue.
